...
<Layout>
  <div class="flex flex-col">
    <div class="nav-text-color">导入Kindle读书笔记</div>
    <!-- 图书信息 -->
    ...
    <div class="flex flex-col note-write-form">
      <div class="flex content-center my-4">
        <div for="comment">粘贴Kindle笔记内容</div>
      <a href="https://www.bilibili.com/video/BV1RW411h7K9/?spm_id_from=333.337.search-card.all.click" class="text-sky-600">(如何导出Kindle读书笔记?)</a>
      </div>
...
<!--      </div>-->
      ...
      <input type="button" value="导入Kindle读书数据" @click="submit">
    </div>
  </div>
</Layout>
</template>
...
export default {
  ...
  methods: {
    submit() {
      ...
      api.importKindle(params).then((resp) => {
        stopLoading()
        showSuccess('导入成功')
